...
from models.dfi_omnistereo.dfi_model import DFIModel
...
def train(cfg: DictConfig):
    ...
    if not cfg.debug:
        ...
        wandb.init(project="DFI-OmniStereo", name=cfg.exp_name, config=config_wandb)
    ...
    model = DFIModel(cfg, model_state, num_steps)
    ...
    for epoch in range(cfg.epochs):
        ...
        if not cfg.debug:
            ...
            if mae < best_mae:
                ...
                filename = f"models/dfi_omnistereo/pretrained_models/{cfg.model_name}_{cfg.exp_name}_{epoch:02d}_{mae:.2f}.pth"
...
if __name__ == '__main__':
    ...
    bool_args = [
        'debug', 'augmented_gt', 'train_depth_anything', 'data_augmentation',
        'do_photo', 'train_encoder', 'use_silog_loss'
    ]
    str_args = [
        'exp_name', 'dataset_root', 'model', 'restore_ckpt',
        'use_sequence', 'depth_anything_version', 'dataset'
    ]
    int_args = [
    ...
    float_args = [
        'lr', 'depth_anything_lr_factor', 'min_disp_deg', 'max_disp_deg', 'wdecay'
    ]
