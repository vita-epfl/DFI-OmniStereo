...
from omnidirectional_stereo_matching.igev_stereo import IGEVStereo
from conversion import calculate_depth_map, disp_pix_to_disp_deg


class DFIModel(BaseModel):
    """
    DFIModel class for training, validation, and inference of the DFI-OmniStereo model.
    Inherits from BaseModel.
    """
    def __init__(self, cfg, model_state, num_steps=1, mode='train'):
        """
        Initialize the DFIModel.
    ...
    def build_model(self):
        """
        Build the DFI OmniStereo model and wrap it in DataParallel for multi-GPU support.
        """
    ...
    def sequence_loss(self, pred, gt, valid):
        ...
        """
        use_silog_loss = self.cfg.use_silog_loss
        ...
        if use_silog_loss:
            disp_loss += self.silog_loss(disp_init_pred[valid], disp_gt[valid]).mean()
        else:
        ...
        for i in range(n_predictions):
            ...
            if use_silog_loss:
                i_loss = self.silog_loss(disp_preds[i][valid], disp_gt[valid]).mean()
            else:
    ...
    def silog_loss(self, pred, gt, beta=0.15):
        """
        Compute the Scale-Invariant Logarithmic Loss (SiLog Loss).
        ...
        Returns:
        - loss: Computed SiLog loss.
        """
        alpha = 1e-7
        g = torch.log(pred + alpha) - torch.log(gt + alpha)
        Dg = torch.var(g) + beta * torch.mean(g) ** 2
        loss = torch.sqrt(Dg)
        return loss
    ...
    def configure_optimizer(self):
        ...
        """
        # Separate parameters for Depth Anything and IGEV modules
        params_depth_anything = {param for param in self.model.module.depth_anything_extractor.parameters() if param.requires_grad}
        params_igev = {param for param in self.model.parameters() if param not in params_depth_anything and param.requires_grad}

        # Configure learning rates for different parameter groups
        if self.cfg.train_depth_anything:
            params = [
                {'params': list(params_depth_anything), 'lr': self.cfg.lr / self.cfg.depth_anything_lr_factor},
                {'params': list(params_igev)}
            ]
        else:
            params = list(params_igev)
