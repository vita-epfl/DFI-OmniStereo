...
from omnidirectional_stereo_matching.update import BasicMultiUpdateBlock
from omnidirectional_stereo_matching.extractor import MultiBasicEncoder
from omnidirectional_stereo_matching.geometry import Combined_Geo_Encoding_Volume
from omnidirectional_stereo_matching.submodule import *
import os
import sys
...
from depth_anything_extractor import DepthAnythingFeature, DepthAnythingExtractor
...
class IGEVStereo(nn.Module):
    def __init__(self, cfg):
        ...
        self.depth_anything_extractor = DepthAnythingExtractor(self.cfg)
        ...
        self.feature = DepthAnythingFeature(self.cfg)
        self.stem_2da = nn.Sequential(
    ...
    def forward(self, image_bottom, image_top, iters=12, flow_init=None, test_mode=False):
        ...
        with autocast('cuda', enabled=self.cfg.mixed_precision):
            image_bottom_depth_anything = ((image_bottom + 1.0) / 2.0)
            image_top_depth_anything = ((image_top + 1.0) / 2.0)
            rel_depth_bottom, out_bottom = self.depth_anything_extractor(image_bottom_depth_anything)
            rel_depth_bottom = rel_depth_bottom.unsqueeze(1)
            rel_depth_bottom = nn.functional.interpolate(rel_depth_bottom, size=(height, width), mode='bilinear', align_corners=True)
            rel_depth_top, out_top = self.depth_anything_extractor(image_top_depth_anything)
            rel_depth_top = rel_depth_top.unsqueeze(1)
            rel_depth_top = nn.functional.interpolate(rel_depth_top, size=(height, width), mode='bilinear', align_corners=True)
            features_bottom = self.feature(out_bottom, height, width)
            features_top = self.feature(out_top, height, width)

            stem_2b = self.stem_2da(rel_depth_bottom)
            stem_2t = self.stem_2da(rel_depth_top)
            ...
            cnet_list = self.cnet(image_bottom, num_layers=self.cfg.n_gru_layers)
